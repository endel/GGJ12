/**
 * 
 * HACED FLX WITH CUSTOM BACKGROUND
 * 
 */
package org.flixel
{
	...
	public class FlxGbkp
	{
		...
		static private var containerDia:Sprite;
		static private var bitmapDay:Bitmap;
		static private var _mask:flash.display.Sprite;
		static private var content:flash.display.Sprite;
		static private var bitmapNoite:Bitmap;
		...
		static internal function init(Game:FlxGame, Width:uint, Height:uint, Zoom:Number):void
		{
			...
			bitmapNoite = new bgNoite();
			bitmapDay = new bgDia();
			
			containerDia.addChild(bitmapNoite);
			
			content.addChild(containerDia)
			content.addChild(_mask)
			containerDia.mask = _mask;
		...
		static internal function lockCameras():void
		{
			...
			while (i < l)
			{
				...
				var posMask:int = (Player.facing == FlxObject.LEFT) ? _mask.width : 0;
				TweenLite.to( _mask, .2, { x:(Player.posX + 32) - _mask.width + posMask} );
				containerDia.mask = _mask;
				cam.buffer.draw(bitmapDay);
				cam.buffer.draw(content);
